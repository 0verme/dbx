...
- The call only reads the stored connection config. It does not connect.
...
- **The connection must already be open.** DBX does not connect on a plugin's behalf, and it does not create one for the plugin.
